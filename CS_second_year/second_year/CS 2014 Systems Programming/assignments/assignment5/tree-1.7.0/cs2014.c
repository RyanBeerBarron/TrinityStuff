...
extern char *endcode;
struct tm *day_of_week;
char* happy_friday = "HAPPY FRIDAY!!! ";
...
off_t friday_listdir(char *d, int *dt, int *ft, u_long lev, dev_t dev)
{
  ...
  while(*dir) 
  {
    ...
    if(day_of_week->tm_wday == 5)
      printit(happy_friday);
    printit(path);
...
off_t friday_rlistdir(char *d, int *dt, int *ft, u_long lev, dev_t dev)
{
  ...
  friday_r_listdir(dir, d, dt, ft, lev);

  return size;
}

void friday_r_listdir(struct _info **dir, char *d, int *dt, int *ft, u_long lev)
{
  char *path;
  long pathsize = 0;
