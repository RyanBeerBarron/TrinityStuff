...
{  
  ...
  void collide(Computer tp)
  {
    if(dy<0)
    {
      if(y-radius <= tp.ypos+PADDLEHEIGHT && y+radius>tp.ypos && x>= tp.xpos && x <= tp.xpos+PADDLEWIDTH)
        dy=-dy;
    }
    if(dy>0 && y-radius<tp.ypos+PADDLEHEIGHT)
    {
      y=tp.ypos+PADDLEHEIGHT+radius;
    }  
  }
}
